...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/19 11:38:49 by larmogid          #+#    #+#             */
/*   Updated: 2023/04/19 11:38:57 by larmogid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_map	*read_map(char *file_name)
{
	...
	map->coordinates = init_coord(map->max_x, map->max_y);
	if (!map->coordinates)
	{
		free(map);
		return (NULL);
...
static int	get_width(char *file_name)
{
	...
	if (!line)
		return (0);
	width = (int)ft_get_word_count(line, ' ');
	free(line);
	while (1)
	{
		line = get_next_line(fd);
		if (line == NULL)
			break ;
		new_width = (int)ft_get_word_count(line, ' ');
		if (width != new_width)
			return (0);
		free(line);
	}
